...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($input_password)){
        $salary_err = "Please enter the password.";     
    } else{
        $salary = $input_salary;
    }
    ...
    if(empty($name_err) && empty($email_err) &&empty($address_err) && empty($password_err)){
        ...
        $sql = "UPDATE employees SET name=?, email=?, address=?, password=? WHERE id=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssi", $param_name, $param_email, $param_address, $param_password, $param_id);
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header">
                        <h2 style="font-weight: bold;">Update Record</h2>
                    </div>
